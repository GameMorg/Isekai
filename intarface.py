...
class Item:
    def __init__(self, box):
        ...
        self.color_pass = (50, 255, 100)
        self.item_mouse = False

    def draw(self, screen):
        draw.rect(screen, self.color_pass, self.rect)

    def update(self, mouse_pos, mouse_press, box_arr):
        if self.rect.x < mouse_pos[0] < self.rect.x + self.rect.w and \
                self.rect.y < mouse_pos[1] < self.rect.y + self.rect.h:  # pos mouse on item
            self.color = (0, 0, 0)
            ...
            if mouse_press[0] == 1:
                ...
                if self.item_mouse:
                    self.item_mouse = False
                    for y in range(len(box_arr)):
                        for x in range(len(box_arr[y])):
                            if box_arr[y][x].rect.x < mouse_pos[0] < box_arr[y][x].rect.x + box_arr[y][x].rect.w and \
                                    box_arr[y][x].rect.y < mouse_pos[1] < box_arr[y][x].rect.y + box_arr[y][x].rect.h and \
                                box_arr[y][x].item == False:
                                self.box = box_arr[y][x]
                                self.box.item = True
...
def interface(screen, display_widht, display_height):
    ...
    for num_box_y in range(5):
        ...
        for num_box_x in range(10):
            ...
            item_box.rect.x = (intarface_bg.x + 500) + (item_box.rect.w + 5) * num_box_x
    ...
    for num in range(5):
        ...
        item = Item(box_x)
        box_x.item = True
        item.color_pass = (50 * num, 50 * num, 50 * num)
